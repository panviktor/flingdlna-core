...
const HEARTBEAT_TYPE_PING: &str = "PING";
const HEARTBEAT_TYPE_PONG: &str = "PONG";
    ...
    S: Write + Read,
{
    ...
    pub fn receive(&self) -> Result<CastMessage, Error> {
        ...
        if message_buffer.is_empty() {
            loop {
                ...
                if self.maybe_handle_heartbeat(&message)? {
                    continue;
                }
                return Ok(message);
            }
    ...
    fn maybe_handle_heartbeat(&self, message: &CastMessage) -> Result<bool, Error> {
        if message.namespace != HEARTBEAT_NAMESPACE {
            return Ok(false);
        }

        let payload = match message.payload {
            CastMessagePayload::String(ref payload) => payload,
            _ => return Ok(false),
        };

        let reply = serde_json::from_str::<serde_json::Value>(payload)?;
        ...
        if message_type != HEARTBEAT_TYPE_PING {
            return Ok(true);
        }

        let pong = serde_json::json!({ "type": HEARTBEAT_TYPE_PONG }).to_string();
        self.send(CastMessage {
            ...
            payload: CastMessagePayload::String(pong),
        })?;
