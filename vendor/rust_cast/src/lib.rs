...
use std::{borrow::Cow, net::{TcpStream, ToSocketAddrs}, sync::Arc, time::Duration};
...
impl<'a> CastDevice<'a> {
        ...
        S: Into<Cow<'a, str>>,
    {
        ...
        // Resolve address and connect with timeout (5 seconds instead of default ~60s)
        let addr = (host.as_ref(), port)
            .to_socket_addrs()?
            .next()
            .ok_or_else(|| Error::Internal("Failed to resolve address".to_string()))?;
...
impl ServerCertVerifier for NoCertificateVerification {
    ...
    fn verify_tls12_signature(
        ...
        // Skip signature verification entirely for Chromecast devices
        // Some devices (e.g., LG webOS TVs) use X.509 v1 certificates which rustls doesn't support
